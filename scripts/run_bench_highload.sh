...
# High-load benchmark script for HTTP/2 vs HTTP/3 performance comparison (高負荷版)
# Tests 4 network conditions: (0/0), (50/0), (100/1), (150/3)
# Features: High load testing, increased requests, more connections, aggressive parameters

echo "================================================"
echo "HTTP/2 vs HTTP/3 High-Load Performance Benchmark"
echo "================================================"
...
# High-load benchmark parameters (大幅に増加)
REQUESTS=500000       # 総リクエスト数（200000→500000に増加）
CONNECTIONS=200       # 同時接続数（100→200に増加）
THREADS=50           # 並列スレッド数（20→50に増加）
MAX_CONCURRENT=200   # 最大同時ストリーム数（100→200に増加）
REQUEST_DATA="High-load benchmark test with extended payload for maximum stress testing - HTTP/2 vs HTTP/3 performance comparison under extreme conditions with realistic data simulation for comprehensive analysis"  # サイズ: 約250バイト

# Fair comparison parameters (高負荷版)
WARMUP_REQUESTS=50000   # ウォームアップ用リクエスト数（20000→50000に増加）
MEASUREMENT_REQUESTS=450000  # 測定用リクエスト数（180000→450000に増加）
CONNECTION_WARMUP_TIME=10  # 接続確立後の待機時間（5→10秒に増加）
...
echo "================================================"
echo "Parameters:"
echo "  Total Requests: $REQUESTS (高負荷版)"
echo "  Connections: $CONNECTIONS (高負荷版)"
echo "  Threads: $THREADS (高負荷版)"
echo "  Max Concurrent Streams: $MAX_CONCURRENT (高負荷版)"
...
echo "    - Warmup Requests: $WARMUP_REQUESTS (高負荷版)"
echo "    - Measurement Requests: $MEASUREMENT_REQUESTS (高負荷版)"
echo "    - Connection Warmup Time: ${CONNECTION_WARMUP_TIME}s (高負荷版)"
echo "  High Load Testing: Enabled"
echo "    - Estimated measurement time: ~8-12 minutes per test case"
echo "    - Protocol separation: 60 seconds between HTTP/2 and HTTP/3"
echo "    - Extended timeouts: 120 seconds for connections"
echo "    - Aggressive parameters for stress testing"
echo "    - Server capacity and stability monitoring"
echo "================================================"
...
# Function to run HTTP/2 benchmark with h2load (高負荷版)
run_http2_bench() {
    local delay=$1
    local loss=$2
    ...
    echo "Running HTTP/2 high-load benchmark (${delay}ms delay, ${loss}% loss)..."
    
    # Clear log file and add header
    echo "=== HTTP/2 HIGH-LOAD BENCHMARK RESULTS ===" > $log_file
    ...
    echo "Establishing HTTP/2 connections for high-load testing..."
    ...
    h2load -n $WARMUP_REQUESTS -c $CONNECTIONS -t $THREADS -m $MAX_CONCURRENT \
        ...
        --header "User-Agent: h2load-highload-benchmark-warmup" \
        --data "$temp_data_file" \
    ...
    h2load -n $MEASUREMENT_REQUESTS -c $CONNECTIONS -t $THREADS -m $MAX_CONCURRENT \
        ...
        --connection-active-timeout 120 \
        --connection-inactivity-timeout 120 \
        --header "User-Agent: h2load-highload-benchmark-measurement" \
        --data "$temp_data_file" \
        --log-file "$csv_file" \
        https://$SERVER_IP/echo >> $log_file 2>&1
    ...
    echo "" >> $log_file
    echo "=== HIGH-LOAD BENCHMARK SUMMARY ===" >> $log_file
    echo "Protocol: HTTP/2" >> $log_file
    echo "Benchmark Type: High Load" >> $log_file
    ...
    echo "HTTP/2 high-load results saved to $log_file"
    echo "HTTP/2 high-load CSV data saved to $csv_file"
}

# Function to run HTTP/3 benchmark with h2load (高負荷版)
run_http3_bench() {
    local delay=$1
    local loss=$2
    ...
    echo "Running HTTP/3 high-load benchmark with h2load (${delay}ms delay, ${loss}% loss)..."
    
    # Clear log file and add header
    echo "=== HTTP/3 HIGH-LOAD BENCHMARK RESULTS ===" > $log_file
    log_network_conditions $delay $loss $log_file
    ...
    # Fair comparison: Establish connections first, then measure
    echo "Establishing HTTP/3 connections for high-load testing..."
    echo "=== CONNECTION ESTABLISHMENT PHASE ===" >> $log_file
    ...
    if grep -q "succeeded, 0 failed" $log_file; then
        ...
        if grep -q "Application protocol: h3" $log_file; then
            echo "✓ h2load HTTP/3 high-load benchmark completed successfully (confirmed HTTP/3)"
            ...
            echo "✓ h2load high-load benchmark completed successfully (protocol unclear)"
        ...
        echo "HTTP/3 high-load results saved to $log_file"
        echo "HTTP/3 high-load CSV data saved to $csv_file"
        return 0
    else
        echo "h2load HTTP/3 high-load failed"
        return 1
    fi
}
...
for test_case in "${test_cases[@]}"; do
    ...
    echo ""
    echo "================================================"
    echo "High-load test case: ${delay}ms delay, ${loss}% loss"
    echo "================================================"
    ...
    echo "Running high-load benchmarks..."
    run_http2_bench $delay $loss
    
    echo "Waiting 60 seconds between protocols (high-load mode)..."
    sleep 60
    
    # Run HTTP/3 benchmark with h2load
    run_http3_bench $delay $loss
    
    echo "Completed high-load test case: ${delay}ms delay, ${loss}% loss"
    echo ""
done

echo "================================================"
echo "All high-load benchmarks completed!"
...
echo "High-load benchmark complete! Check the reports and graphs in $LOG_DIR"
'

echo "================================================"
echo "高負荷ベンチマーク完了: $(date)"
echo "================================================"
...
echo "================================================"
echo "高負荷完全自動化完了: $(date)"
echo "高負荷ベンチマーク + グラフ生成が正常に完了しました"
