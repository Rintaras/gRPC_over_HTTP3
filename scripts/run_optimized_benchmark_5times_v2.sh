#!/bin/bash
# 最適化ベンチマークを5回連続実行するスクリプト v2
# 既存の動作するスクリプトをベースに最適化を適用
...
for run in $(seq 1 $TOTAL_RUNS); do
    ...
    # 最適化ベンチマーク実行
    if ./scripts/run_japanese_benchmark.sh; then
        # 実行成功
        RUN_END_TIME=$(date +%s)
        ...
        # 最新のログディレクトリを記録
        LATEST_DIR=$(ls -td logs/japanese_benchmark_* 2>/dev/null | head -1 || echo "logs/japanese_benchmark_$(date +%Y%m%d_%H%M%S)")
