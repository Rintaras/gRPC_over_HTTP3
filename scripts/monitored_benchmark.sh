#!/bin/bash
# ネットワークとシステムリソースを同時に監視するベンチマークツール
...
main() {
    # ログディレクトリ作成
    LOG_DIR="logs/monitored_benchmark_$(date +%Y%m%d_%H%M%S)"
    mkdir -p "$LOG_DIR"
    
    log_info "監視付きベンチマーク開始"
...
EOF
    ...
    docker exec grpc-client bash -c "
        ...
        LOG_DIR=\"/logs/monitored_benchmark_\${NOW}\"
        mkdir -p \$LOG_DIR
        ...
        THREADS=10
        MAX_CONCURRENT=50
        REQUEST_DATA=\"監視付きベンチマーククライアントからのテスト - HTTP/2 vs HTTP/3 性能比較テスト用の現実的なデータペイロード\"
        ...
        echo \"監視付きベンチマーク実行結果\" >> \$LOG_DIR/benchmark_summary.txt
        echo \"================================================\" >> \$LOG_DIR/benchmark_summary.txt
        ...
        echo \"  監視機能: 有効\" >> \$LOG_DIR/benchmark_summary.txt
        echo \"================================================\" >> \$LOG_DIR/benchmark_summary.txt
